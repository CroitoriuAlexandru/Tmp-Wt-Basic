...
TailwindTheme::TailwindTheme(const std::string& name)
  : WTheme(),
    name_(name)
{ }
...
std::vector<WLinkedCssStyleSheet> TailwindTheme::styleSheets() const
{
...
void TailwindTheme::apply(WWidget *widget, WWidget *child, int widgetRole) const
{
...
void TailwindTheme::apply(WWidget *widget, DomElement& element, int elementRole)
  const
{
  ...
  case DomElementType::BUTTON:
    if (creating) {
      element.addPropertyWord(Property::Class, Wt::WString::tr("button").toUTF8());
...
void TailwindTheme::applyValidationStyle(WWidget *widget,
...
bool TailwindTheme::canBorderBoxElement(const DomElement& element) const
{
  return true;
}
