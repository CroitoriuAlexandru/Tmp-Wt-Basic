	...
	: WContainerWidget()
{
	...
	auto messageResources = app->messageResourceBundle().keys("");

	// std::cout << "\n\n";
	// for(auto& key : messageResources){
	// 	std::cout << "\n key: " << key << "\n";
	// }
	

	// app->messageResourceBundle().use("./resources/translations/test-template.xml", true);
	...
	// app->messageResourceBundle().resolveKey(locale, "test-template");
	// for(auto& key : messageResources){
	// 	std::cout << "\n key: " << key << "\n";
	// }
	auto temp = addWidget(std::make_unique<Wt::WTemplate>(tr("test")));
	temp->bindWidget("controls", std::make_unique<TemplateEditorDialog>("test", "test"));
